...
![测试路径行为](https://raw.githubusercontent.com/yunnysunny/nodebook/master/images/zh-tw/node_basic_rout_test.png "")
...
![回传](https://raw.githubusercontent.com/yunnysunny/nodebook/master/images/zh-tw/node_basic_rout_url.png "")
...
![index_url](https://raw.githubusercontent.com/yunnysunny/nodebook/master/images/zh-tw/node_basic_rout_url_index.png "")

![test_url](https://raw.githubusercontent.com/yunnysunny/nodebook/master/images/zh-tw/node_basic_rout_url_test.png "")

![default_url](https://raw.githubusercontent.com/yunnysunny/nodebook/master/images/zh-tw/node_basic_rout_url_default.png "")

![error_url](https://raw.githubusercontent.com/yunnysunny/nodebook/master/images/zh-tw/node_basic_rout_url_error.png "")
...
前面已经了解如何读取本地端档案，接下来将配合http服务器路由，让每个路由都能够输出相对应的静态 html 档案。首先新增加几个静态html 档案，准备一个包含基本路由功能的http 服务器
...
readFile的回应函数里面加入页面输出，让浏览器可以正确读到档案，在这边我们设定读取的档案为html 静态档案，所以 Content-type 设定为 **text/html**。读取到档案的内容，将会正确输出成 html 静态档案。
...
到这边为止基本的程序内容都已经完成，剩下一些细节的调整。首先路径上必须做调整，目前的静态档案全部都放置于**static 资料夹** 底下，设定一个变数来记住资料夹位置。
...
### 3.5 node.js http GET 资料采集


http 服务器中，除了路由之外另一个最常使用的方法就是采集GET资料。本单元将会介绍如何透过基本http 服务器采集浏览器传来的要求，采集GET资料。

在http 协定中，GET 参数都是藉由URL从浏览器发出要求送至服务器端，基本的传送网址格式可能如下，
...
透过 query string ，使用parse 这个方法将资料转换成javascript对象，就表示 GET 的资料已经被服务器端正式采集下来，
