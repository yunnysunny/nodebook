...
> 为了构图方便，这里简单认为各个 `req_count` 指标中只有 `status` 这一个 label。
...
需要注意，我们 grafana 只支持瞬时向量和标量，不支持范围向量，但是这并不代表范围向量是没有用的。对于计数器类型的指标来说，在 grafana 中将会呈现出一条单调递增的直线（如图 **图 14.2.1.1.1** 所示），不是很好对其进行分析，这时候你可以使用 Prometheus 内置函数先将计数器先转化成范围向量，然后再对范围向量内的数值做运算转成一个瞬时向量，比如说 rate 函数就是将范围向量转成瞬时向量的常用操作，这样你就可以分析单位时间内的变化速率了。
