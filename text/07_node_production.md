...
```  
**代码 7.3.2 不使用测试框架的测试用例**  
接着他又写了减法函数、乘法函数、除法函数，但是随着模块的增加，他逐渐的意识到一个问题，如果按照代码7.3.2的模式的话，一则输出格式比较乱，而且测试结果没有最终统计信息，不能一下子得出成功数和失败数。于是乎他去网上找资料，然后他就发现了大名鼎鼎的 `mocha`。  
...
```  
**代码 7.3.4 文件calculator_test.js部分代码**  
...
ReferenceError: describe is not defined                                                                                        
    ...
    at node.js:929:3
```  
**输出 7.3.1 命令node calculator_test.js的输出**  
...
e:\kuaipan\code\node\myapp\chapter7\src\test\mocha>mocha calculator_test.js                                                    
  ...
  1 passing (7ms)
```
**输出 7.3.2 命令mocha calculator_test.js的输出**  
